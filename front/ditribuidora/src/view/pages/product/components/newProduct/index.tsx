...
import { useNewProductController } from "./useNewProductController";


export function NewProduct() {

  const { errors, handleSubmit, register, control, dataCategory, dataSupplier} = useNewProductController();

  return (
    <form onSubmit={handleSubmit} className="flex w-full max-w-md mt-10 flex-col flex-wrap justify-between gap-8 p-4">
      <p className="text-[#1C1D22] tracking-light text-[32px] font-bold leading-tight min-w-72">Criar novo produto</p>
      <TextField 
        size="small" 
        label="Nome" 
        variant="outlined" 
      ...
      <Button className="bg-gray-950 hover:bg-gray-700" onClick={handleSubmit}>Salvar</Button>
    </form>
  )
}
